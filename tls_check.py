    ...
    for version, cipher in results.items():
        if cipher:
            ...
            if version=='TLSv1' or version=='TLSv1.1':
                print("*** /\ /\ WARNING Isecure TLS version /\ /\ ***")
                tls_fail=True
        else:
    ...
    if tls12 is not None:
        ...
        for c in ciphers:
            if '-AES256-' in c:
                ...
                nsa_c=" "
                nsa_enc=False
            print("  {}     {}".format(c, nsa_c))
    else:
        print("\nSkipping TLS 1.2 cipher scan because this Python/ssl build lacks TLSVersion enums.")

    tls13 = TLS.get("TLSv1.3")
    if tls13 is not None:
        print("\nScanning ciphers for TLS 1.3 on {}:{}...\n".format(target, args.port))
        ciphers = get_supported_ciphers(target, port=args.port, tls_version=tls13, use_sni=use_sni)
        print("TLS 1.3 supported ciphers ({}):".format(len(ciphers)))
        for c in ciphers:
            if '-AES256-' in c:
                nsa_c="<< NSA Approved"
            else:
        ...
        print("\nSkipping TLS 1.3 cipher scan because this Python/ssl build lacks TLSVersion enums.")

    print("Summary =------------------------------------------------------------------------------")
    print(f" TLS Version < 1.2 {tls_fail}")
    print(f" Are all ciphers NSA apprved: {nsa_enc}")
    print(f" Check above for more details")
    print("---------------------------------------------------------------------------------------")
